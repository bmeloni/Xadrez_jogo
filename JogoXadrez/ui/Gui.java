...
public class Gui extends JFrame {
	...
	final private Point TAM_JANELA = new Point (800, 620);	/// tamanho total da janela
	final private Point INICIO_TABULEIRO = new Point (10, 37);	/// início do tabuleiro
	final private Point TAM_LOG = new Point (180, (int) TAM_TABULEIRO);	/// tamanho do log de movimentos
	final private Point INICIO_LOG = new Point (0, 0);	/// início do log de movimentos
	...
	/**
	 * Ctor: constroi a tela principal
	 */
	private Gui () {
		setTitle ("Xadrez");
		setSize ((int) TAM_JANELA.getX (), (int) TAM_JANELA.getY ());		// 800x600
	...
	private void montaTabuleiro (JPanel panel) {
		...
		for (byte i = 0; i < 8; i++) {
			...
			for (byte j = 0; j < 8; j++) {
				...
				// cor do fundo, branco ou rosa
				Color cor = ((i + j) % 2 == 0) ? Color.WHITE : Color.PINK;
	...
	private void montaQuemJoga (JPanel panel) {		
		quem_joga = new JLabel ("Jogador Branco, você começa o jogo!", SwingConstants.CENTER);
	...
	private void menu () {
		...
		Renomear.addActionListener (new ActionListener() {
			...
			public void actionPerformed (ActionEvent event) {
				...
				String novo_nome = JOptionPane.showInputDialog ("Novo nome do jogador " + j);
		...
		if (!ObServer.estaEmRede ()) {
			...
			Empatar.addActionListener (new ActionListener() {
				...
				public void actionPerformed (ActionEvent event) {
					...
					int n = JOptionPane.showConfirmDialog (Gui.getTela (), Xadrez.getDaVez () + " está propondo um empate. Aceita?", "Proposta de empate", JOptionPane.YES_NO_OPTION);
	...
	private void montaRelojs (JPanel panel) {
		...
		// relógio do outro jogador está parado
	...
	private void novoJogo () {
		...
		if (!motor.possoJogar ())
			...
			quem_joga.setText ("Jogador Branco, você começa o jogo!");
	...
	public void trocaJogador (Jogador J) {
		...
		String [] str = {
				...
				"não me desaponte!",
				"sua vez",
				"pense bem na próxima jogada",
				"joga aí",
				"agora é você, não desista!",
				"sua vez, Jogue com atenção!",
				"fique sempre atento ao rei",
				"cuidado aí, rapaz!",
				"agora é a sua vez"
		};
		...
		quem_joga.setForeground (Color.BLACK);
		if (motor.possoJogar ())
			quem_joga.setText (J + ", " + str[rand.nextInt (12)]);
		else
			quem_joga.setText ("Esperando jogada, aguarde!");
	}
	/**
	...
	public void xeque (Jogador J, boolean refresh) {
		...
		if (motor.possoJogar ())	// é offline ou online e minha vez
			quem_joga.setText (J + " está em xeque, faça alguma coisa!!");
	...
	public void mate (Jogador J) {
		quem_joga.setForeground (Color.RED);
		quem_joga.setText ("Xeque mate! " + J + " seu rei já era!");
		...
		String pastor = "";
		if (rodadas == 4)
			pastor = "Po, " + J + ", de xeque do pastor\n";
	...
	/**
	 * Pega do usuário o endereço do servidor a conectar.
	 * Endereço padrão é 'localhost'.
	 */
	...
	public void falhaComunicacao () {
		JOptionPane.showConfirmDialog (this, "A conexão falhou, e a culpa não é do programa\nAcabou o jogo...", "Conexão perdida",  JOptionPane.OK_CANCEL_OPTION);
		System.exit (0);
	}
