...
public class Jogador implements Serializable {
	...
	public void novoJogo () {
		...
		// e o reizão xD
		reis = (Rei) tab.getCasa (linha, 4).getPeca ();
	...
	public void updateRei () {
		...
		aux.addAll (reis.Roques (roque_maior, roque_menor));
		aux.addAll (reis.possiveisMovimentos ());
		// se não tem movimento possível, indice_comeco e indice_fim serão iguais
		reis.setIndiceComeco (movs.size ());
		movs.addAll (aux);
		reis.setIndiceFim (movs.size ());
	}
	...
	public boolean estaXeque () {
		return Tabuleiro.getTabuleiro ().getCasa (reis.getCoord ()).getDominio ().ameaca (cor);
	}
	public boolean estaXeque (Simulador sim) {
		return sim.getCasa (sim.getReis (this.cor)).getDominio ().ameaca (cor);
	}
	/**
