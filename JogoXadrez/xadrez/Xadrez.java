...
public class Xadrez {
	...
	// diz se já clicou em uma casa válida
		...
		if (partida && possoJogar ()) {
			...
			if (casa_marcada == false) {
				...
				for (Movimento m : mov) {
					...
					// descolore os quadrados de possibilidade
					m.unPrintPossivel ();
				}
				if (a_ser_feito != null) {
	...
	private void processaMovimento (Movimento a_ser_feito) {
		...
		/*  snapshots  */
		P.snap_atual++;
		// se desfez o movimento, remove os que tinha pra frente
	...
	private void trocaJogador (boolean refresh) {
		if (partida) {
			if (!refresh) {
				// peões que tinham andado 2 casas agora não podem mais ser tomados
				jogador_da_vez.updatePiaums ();
			}
			
			// relógio da vez para - vez do outro agora
